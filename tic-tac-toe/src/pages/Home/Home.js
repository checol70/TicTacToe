...
class Home extends Component {
    ...
    makeMove = (i, j) => {
        ...
        let draw = state.board[0].concat(state.board[1]).concat(state.board[2]).indexOf(0) === -1
        if (winner === 0 && !draw) {
    ...
    test = (board, recursionLevel, previousLevel, minMax) => {
        ...
        if (recursionLevel < 3 && !currentLevel.chances.includes(-1)) {
            ...
            for (let i = 0; i < possibles.length; i++) {
                if (chances[i] === 0) {
                    const check = this.test(possibles[i], parseInt(recursionLevel.toString()), currentLevel)
    ...
    getIdealChoice(results) {
        const minResults = results.twoMoves.map((move)=>{
            return Math.min(...move.chances);
        })
    ...
    computerMakeMove = (state) => {
        ...
        const filledPlaces = state.board.map(row => row.map(value => Math.abs(value)));
        const totalMoves = filledPlaces.reduce((a, b) => a + b.reduce((c, d) => c + d, 0), 0);
        if (totalMoves === 1) {
            if (state.board !== [[0, 0, 0], [0, -1, 0], [0, 0, 0]]) {
                state.board[1][1] = 1;
                return state;
            }
